...
async def fetch_and_update_cve_data(semaphore, data_semaphore, session, tcp_port, port_descriptions, existing_data):
    ...
    while attempt < max_retries:
        try:
            async with semaphore:
                ...
                port_entry = next((item for item in existing_data["vulnerabilities"] if item["port"] == tcp_port), None)
                if not port_entry:
...
async def update_cve_data(data_semaphore, cve_name, cve_description, tcp_port, port_descriptions, existing_data):
    async with data_semaphore:
        current_year = datetime.now().year
...
async def main(existing_data):
    ...
    async with aiohttp.ClientSession() as session:
        tasks = [fetch_and_update_cve_data(semaphore, data_semaphore, session, port, port_descriptions, existing_data) for port in range(start_port, end_port + 1)]
